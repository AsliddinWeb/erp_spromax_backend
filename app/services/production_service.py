...
class ProductionService:
    ...
    def delete_machine(self, machine_id: UUID) -> bool:
        ...
        self.db.execute(text("""
            DELETE FROM maintenance_logs WHERE request_id IN (
                SELECT id FROM maintenance_requests WHERE machine_id = :mid
            )
        """), {"mid": str(machine_id)})
        self.db.execute(text("DELETE FROM maintenance_requests WHERE machine_id = :mid"), {"mid": str(machine_id)})
