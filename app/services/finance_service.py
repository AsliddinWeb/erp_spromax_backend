...
class FinanceService:
    ...
    def _get_opening_balance(self, before_date: datetime) -> Decimal:
        """
        Berilgan sanadan OLDINGI barcha tranzaksiyalar asosida
        boshlang'ich balansni hisoblash.

        Formula: SUM(income) - SUM(expense) before_date gacha
        """
        income_before = self.transaction_repo.get_total_by_type(
            'income',
            end_date=before_date
        )
        expense_before = self.transaction_repo.get_total_by_type(
            'expense',
            end_date=before_date
        )
        return income_before - expense_before
    ...
    def get_cash_flow(
        ...
        # BUG FIX: opening_balance endi start_date gacha bo'lgan
        # barcha tranzaksiyalardan hisoblanadi
        opening_balance = self._get_opening_balance(before_date=start_date)
